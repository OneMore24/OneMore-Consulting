# 🏢 OneMore Consulting

![Repo Size](https://img.shields.io/github/repo-size/tu-usuario/onemore-consulting?style=flat-square&color=6a11cb)
![Latest Commit](https://img.shields.io/github/last-commit/tu-usuario/onemore-consulting?style=flat-square&color=2575fc)

---

## 👥 Quiénes Somos
**OneMore Consulting** es una firma de desarrollo y consultoría de software orientada a la creación de soluciones tecnológicas integrales. Nos especializamos en la estructuración, diseño y despliegue de sistemas eficientes, asegurando la calidad y escalabilidad en cada etapa del ciclo de vida del software para satisfacer las necesidades críticas de nuestros clientes.

---

## 📂 Estructura del Repositorio
Nuestra arquitectura de gestión se divide en los siguientes espacios de trabajo:

* **`Clientes/`**: Información comercial, gestión de stakeholders y requerimientos por cuenta.
* **`Desarrollo/`**: Código fuente, diseño de arquitectura y bases de datos de los proyectos.
* **`Documentos/`**: Repositorio de políticas, directrices operativas y planes de gestión.
* **`Linea_Base/`**: Control de versiones y trazabilidad de los hitos entregables.

---

## 🛠️ Stack Tecnológico
> *Nota: Nuestro stack tecnológico se encuentra actualmente en un **proceso de evaluación y definición estratégica**. El equipo está analizando los requerimientos arquitectónicos para seleccionar las herramientas definitivas que garanticen el mejor rendimiento.*

| Categoría | Tecnologías (En Evaluación) |
| :--- | :--- |
| **Lenguaje Base** | ![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white) |
| **Base de Datos** | ![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white) |
| **Metodología** | ![Agil / Tradicional](https://img.shields.io/badge/ÁGIL_/_TRADICIONAL-333333?style=for-the-badge) |

---

## 🚀 Portafolio de Clientes y Proyectos

| Cliente | Proyecto | Estado |
| :--- | :--- | :--- |
| **Clínica Anxiety** | ![APCA](https://img.shields.io/badge/APCA-PROYECTO_PRINCIPAL-FFD700?style=for-the-badge&logoColor=black) | 🟡 **Planificación** |
| Excelentemente | Proyecto GIE | 🟢 En Desarrollo |
| Mente en Calma | Proyecto SNMP | ⚪ Pendiente |
| Clínica San Gabriel | Proyecto VITALIS | 🟢 En Desarrollo |
| Agroexportadora Verde | Proyecto SENTRA | ⚪ Pendiente |
| Corporación Talento | Proyecto PLEC | 🟢 En Desarrollo |
| Grupo Andino Comercial | Proyecto SADE | ⚪ Pendiente |
| Aerolíneas VuelaPerú | Proyecto VUELA_PERU | 🟢 En Desarrollo |

---

## 👤 Equipo de Trabajo (Grupo 01)

| Nombre | Rol |
| :--- | :--- |
| **Alan Ventura** | Jefe de Proyecto / DBA |
| **Luis Arroyo** | Desarrollador Frontend |
| **Luis Machaca** | Analista de Requisitos / Arquitecto de WS |
| **Matias Vincula** | Desarrollador Backend |
| **Ricardo Matamoros** | Diseñador UI/UX |
| **David Aldana** | Analista de Calidad (QA) / Tester |

